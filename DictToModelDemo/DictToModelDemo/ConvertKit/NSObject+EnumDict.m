...
/*
 思路1：利用运行时，首先要遍历参数字典, 如果我们获取得属性列表中包含了字典中的key，
      就利用 KVC 方法赋值，然后就完成了字典转模型的操作。
 */
    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        if ([propertyList containsObject:key]) {
            ...
            if ([obj isKindOfClass:NSClassFromString(@"__NSCFDictionary")]) {
                ...
                // value:user字典 -> User模型
                // 获取模型(user)类对象
                if ([self respondsToSelector:@selector(dictWithModelClass)]) {
                    id mySelf = self;
                    NSString *ivarType = [mySelf dictWithModelClass][key];
